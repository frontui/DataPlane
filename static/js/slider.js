...
;(function ($, f) {
	...
	var Unslider = function() {
		...
		this.init = function(el, opts) {
			this.el = el;
			this.dl = el.children('dl');
			this.max = [(el.outerWidth() || el.parent().outerWidth()), (el.outerHeight() || el.parent().outerHeight())];
			this.items = this.el.children('dd').each(this.calculate);
			console.log("------------_thisdd:"+this.el.children('dd').style.display);
		...
		this.setup = function() {
			...
			this.dl.css({width: (this.items.length * 100) + '%', position: 'relative'});
		...
		this.move = function(index, cb) {
			...
			if(!this.dl.is(':animated')) {
				//  Handle those pesky dots
				_.el.find('.dot:eq(' + index + ')').addClass('active').siblings().removeClass('active');

				this.el.animate(obj, speed).trigger(moveEvent) && this.dl.animate($.extend({left: '-' + index + '00%'}, obj), speed, function(data) {
					_.current = index;
